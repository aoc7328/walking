...
import { translateLines, translateText } from './translate';
...
async function translateChannel(ch: string, target: 'ja' | 'en' | null): Promise<string> {
  if (!ch) return '';
  if (!target) return ch;
  return CHANNEL_MAP[target][ch] ?? translateText(ch, target);
}
...
export async function buildReservationCard(r: Restaurant, ledger: Ledger): Promise<ReservationCardData> {
  ...
  const tr = (text: string) => (target ? translateText(text, target) : Promise.resolve(text));
  ...
  const dietary = dietaryRaw ? (target ? await translateLines(dietaryRaw, target) : dietaryRaw) : '';
